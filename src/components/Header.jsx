...
const Header = () => {
  ...
  return (
    <nav className="w-full flex bg-black justify-between items-center gap-1 lg:px-16 px-6 py-4 sticky top-0 z-50">
      <h1 className="text-white md:text-4xl text-3xl font-bold font-mahoor ">ادد<span className="text-limegreen italic">باکس</span></h1>

      <ul className="lg:flex justify-center items-center gap-8 hidden">
      {navItems.map(({link, path}) => (
        <Link key={path} className="text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-limegreen hover:text-black font-mahoor text-[15px]" to={path} spy={true} offset={-100} smooth={true} >{link}</Link>
      )

      )}
      </ul>

      <button className="bg-limegreen font-mahoor hover:bg-white text-black px-11 py-3 rounded-full font-bold transform hover:scale-105 transition-transform duration-300  cursor-pointer md:flex hidden "><a  href="https://wa.me/989127221655" target="_blank" rel="noopener noreferrer"><span><FaWhatsapp className="text-black text-[20px] " /></span>ارتباط با ما</a></button>
          {/* منو استارت موبایل */}
